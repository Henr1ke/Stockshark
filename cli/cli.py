...
def app_navigate_menus() -> None:
    coordinates = models_dict[args.model]()
...
def navigate_vs_friend(menu_navigator: MenuNavigator) -> None:
...
def navigate_vs_computer(menu_navigator: MenuNavigator) -> None:
...
def find_board_info() -> Optional[Tuple[ndarray, Tuple[int, int]]]:
    for _ in range(60):
...
engine = engines_dict[args.engine]()
agent = agents_dict[args.agent]()

menu_navigation_mode = args.menu_navigation_mode
if menu_navigation_mode is not None and menu_navigation_mode.casefold() == navigate_menus:
    app_navigate_menus()
